...
// Estrutura para armazenar o estado de cada coelho
struct Bunny
{
    float angle;           // Ângulo atual no círculo (movimento)
    float radius;          // Distância do centro
    float base_height;     // Altura de flutuação base (Y)
    float angular_speed;   // Velocidade com que percorre o círculo
    float jump_phase;      // Fase da senoide para o pulo
    float jump_speed;      // Velocidade do pulo
    float jump_height;     // Altura do pulo
    float flip_angle;      // Ângulo atual da cambalhota (eixo X)
    float time_until_flip; // Tempo restante para a próxima cambalhota
    bool is_flipping;      // Estado verificador da cambalhota
    bool can_flip;
};
...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00588758 - Arthur Andrade da Silva", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
for (int i = 0; i < num_bunnies; i++)
    {
        ...
        b.flip_angle = 0.0f;
        b.time_until_flip = 1.0f + ((float)rand() / RAND_MAX) * 6.0f;
        b.is_flipping = false;

        flock_of_bunnies.push_back(b);
    }
    float last_frame_time = (float)glfwGetTime();

    // Ficamos em um loop infinito...
...
#define PLANE 2
        ...
        // Renderizamos as dezenas de coelhos flutuantes
...
// 3. Calcula as posições 3D do coelho (X, Y, Z)
            ...
            // 4. Calcula a direção para onde olhar
            float facing_angle = -b.angle - (3.141592f / 2.0f);

            // ==========================================
            // RENDERIZAÇÃO DO COELHO
            // ==========================================
            glUniform1i(g_object_id_uniform, BUNNY);
